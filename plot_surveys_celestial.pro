;Time-stamp: <2010-06-14 21:47:55 sudeep>
;AUTHOR: Sudeep Das, Princeton University, 2007-2009
;Used in conjuction with savesurveys.pro 

pro plot_surveys_celestial,inputFileName,psFileName,showPossible=showPossible
...
;map[0]=0.
;project onto Mollweide map from G-> C
reproj_healpix,map,map2,coord=2,project=1,size=3
...
reproj_healpix,map,map3,coord=5,project=1,size=3 ;'C'->'C'

;clear out overlapping areas in WMAP map
idx = where(map3 ne 0.)
map2[idx]=0.

reproj_healpix,map0,map4,coord=5,project=1,size=3 ;'C'->'C'
;map4 = map0
...
;for bgColor
map0[*]=1.0
reproj_healpix,map0,map5,coord=5,project=1,size=3 ;'C'->'C'
...
load_ed_colors,mycolors
grid_overlay_sudeep2,image=newmap,ps=psFileName,lon=lon,lat=lat,imcoord = 'C',grcoord='C',/leave_ps_open,$
  color=(where(labelColors eq gridColor))[0],ct=27

hrtodeg=360./24.
label_overlay,rax,decx,mollx,molly,'C','C'
...
labelRA[9]=' '
label_overlay,lon0,lat0*0.,mollx,molly,'C','C'
load_ed_colors,mycolors
xyouts,mollx,molly,labelRA,/norm,charthick=3,charsize=0.8,color=(where(labelColors eq raLabelColor))[0],orientation=90
...
label_overlay,lon0,lat0,mollx,molly,'C','C'
load_ed_colors,mycolors
;xyouts,mollx,molly,labelRA,/norm,charthick=3,charsize=.8,color=(where(labelColors eq raLabelColor))[0],orientation=-55
;now do dec
