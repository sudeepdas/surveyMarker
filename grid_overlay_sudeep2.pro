...
;     
;
;     leave_ps_open: added by sudeep. If set, leaves the ps file open for
;     plotting labels etc. 
;     ct : color table : added by Sudeep
; 
; COMMON BLOCKS:
;     None.
;
...
;======================================================================
;
pro grid_overlay_sudeep2,proj=proj,imcoord=imcoord,grcoord=grcoord, $
                 lon=lon, lat=lat, color=color,ps=ps,image=image, $
                 zbuff=zbuff,user=user,leave_ps_open=leave_ps_open,ct=ct
;
;
; Projection Type
;
...
if keyword_set(ps) then begin
  ...
  device,file=ps
  ;if not(keyword_set(ct)) then ct=27
  ...
  endelse
endif else if keyword_set(user) then begin
...
if not(keyword_set(leave_ps_open)) then begin
